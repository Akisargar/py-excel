...
(function ($, Drupal) {
  ...
  Drupal.behaviors.select2 = {
    attach: function attach(context, setting) {
      ...
      setTimeout(function () {
        $('body').find('select.hs-input').each(function (i, el) {
          var dropdownParent = el.closest('div.hs-fieldtype-select');
          $(el).select2({
            minimumResultsForSearch: Infinity,
          ...
          $(el).on('change', function () {
            $(this).closest('select.hs-input').addClass('is-placeholder select2-hidden-accessible');
          });
        });
      }, 1000);
    }
